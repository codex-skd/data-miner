...
- **Nombre de JAR con versión del cargador**: el artefacto ahora se compila como `data_miner-26.1.2-neoforge-26.1.2.76-1.0.2.jar` (se añade la versión de cargador/NeoForge al nombre del archivo). Empaquetado y documentación; sin cambios de funcionalidad.
...
- Estructura de proyecto alineada con WORKFLOW.md: ramas, nombre del JAR, formato de tags.
- JAR ahora sigue el formato `<mod_id>-<mc_version>-<framework>-<version>.jar`.
- Tags siguen el formato `<mc-version>-neoforge-X.Y.Z`.

### Corregido

- ClassCastException en slow tick detection (servidor).
- Explosión de archivos de incidencias: ahora un único `issues.jsonl` por sesión.
